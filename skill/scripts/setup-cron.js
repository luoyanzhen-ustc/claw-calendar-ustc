...
function main() {
  ...
  if (failedCount > 0) {
    ...
    console.log('手动配置命令:');
    console.log('  openclaw cron add --schedule \'0 2 * * *\' --name \'claw-calendar-daily\' --payload \'{"kind": "agentTurn", "message": "每日索引重建"}\'');
    console.log('  openclaw cron add --schedule \'0 0 * * 1\' --name \'claw-calendar-weekly\' --payload \'{"kind": "agentTurn", "message": "每周总结归档"}\'');
    console.log('');
  } else {
    console.log('✅ 所有 Cron 任务配置成功！\n');
    console.log('下一步:');
    console.log('  1. 运行 `openclaw cron list` 查看任务列表');
    console.log('  2. 等待次日凌晨 2 点检查每日任务执行日志');
    console.log('  3. 或手动触发：openclaw cron run --name \'claw-calendar-daily\'');
    console.log('');
  }
}
